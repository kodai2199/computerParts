...
public class Connect {
	...
	public ArrayList<Cases> loadCases() throws SQLException {
        ...
        String query="SELECT * FROM computerParts.Case";
        ResultSet rs=st.executeQuery(query);
        while (rs.next()) {
            Cases cs;
	...
	public Cases loadCase(int id) throws SQLException {
		Cases c;
		String query = "SELECT * FROM computerParts.Case JOIN Component ON computerParts.Case.IdComponent = Component.IdComponent WHERE computerParts.Case.IdComponent = '" + id + "'";
		ResultSet rs=st.executeQuery(query);
		if (rs.next()) {
			String name = rs.getString("Name");
			double price = rs.getBigDecimal("Price").doubleValue();
